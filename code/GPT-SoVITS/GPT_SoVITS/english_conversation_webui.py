...
import torch
import sys
sys.path.append('audio/repos/GPT-SoVITS')
sys.path.append('audio/repos/GPT-SoVITS/GPT-SoVITS')
...
def get_prompt_to_qwen(ori_text):
    prompt = f'''
    You are a professional English teacher, Please score my English and tell me if I have any language problems, 
    Dont appear redundant output, just dialogue.
    the following is my input: {ori_text}. '''
...
with gr.Blocks(title="ASR_GPT_CTTS WebUI") as app:
    ...
    with gr.Group():       
        gr.Markdown(value=i18n("2 上传并填写参考信息"))
        with gr.Row():
            with gr.Column():
                input_text = gr.Textbox(label="直接输入文本", value=""),
                record = gr.Audio(sources=["upload", "microphone"], type="filepath", label="上传文件/麦克风输入"),
            asr_button = gr.Button(i18n("语音识别"), variant="primary")
            with gr.Column():
                asr_result = gr.Textbox(label="源文本或语音识别结果", value=""),
                translate_result = gr.Textbox(label="翻译结果"),
        asr_button.click(fn=process_input,
            inputs=[input_text[0], record[0]],
            outputs=[asr_result[0], translate_result[0]],
        )
        
    with gr.Group():
        gr.Markdown(value=i18n("3 GPT对话"))
        with gr.Row():    
            gpt_button = gr.Button(i18n("GPT英语对话"), variant="primary")
