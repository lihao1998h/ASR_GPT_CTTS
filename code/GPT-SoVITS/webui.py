...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Tabs():
        with gr.TabItem("0-前置数据集获取工具"):#提前随机切片防止uvr5爆内存->uvr5->slicer->asr->打标
            ...
            with gr.Row():
                open_denoise_button = gr.Button("开启语音降噪", variant="primary",visible=True)
                close_denoise_button = gr.Button("终止语音降噪进程", variant="primary",visible=False)
                denoise_input_dir=gr.Textbox(label="降噪音频文件输入文件夹",value="")
                denoise_output_dir=gr.Textbox(label="降噪结果输出文件夹",value="output/denoise_opt")
                denoise_info = gr.Textbox(label="语音降噪进程输出信息")
        ...
        with gr.TabItem("1-GPT-SoVITS-TTS"):
            ...
            with gr.TabItem("1B-微调训练"):
                ...
                with gr.Row():
                    batch_size = gr.Slider(minimum=1,maximum=40,step=1,label="每张显卡的batch_size",value=default_batch_size,interactive=True)
                    total_epoch = gr.Slider(minimum=1,maximum=25,step=1,label="总训练轮数total_epoch，不建议太高",value=8,interactive=True)
                    text_low_lr_rate = gr.Slider(minimum=0.2,maximum=0.6,step=0.05,label="文本模块学习率权重",value=0.4,interactive=True)
                    save_every_epoch = gr.Slider(minimum=1,maximum=25,step=1,label="保存频率save_every_epoch",value=4,interactive=True)
                    if_save_latest = gr.Checkbox(label="是否仅保存最新的ckpt文件以节省硬盘空间", value=True, interactive=True, show_label=True)
                    if_save_every_weights = gr.Checkbox(label="是否在每次保存时间点将最终小模型保存至weights文件夹", value=True, interactive=True, show_label=True)
                ...
                with gr.Row():
                    batch_size1Bb = gr.Slider(minimum=1,maximum=40,step=1,label="每张显卡的batch_size",value=default_batch_size,interactive=True)
                    ...
                    if_save_latest1Bb = gr.Checkbox(label="是否仅保存最新的ckpt文件以节省硬盘空间", value=True, interactive=True, show_label=True)
                    if_save_every_weights1Bb = gr.Checkbox(label="是否在每次保存时间点将最终小模型保存至weights文件夹", value=True, interactive=True, show_label=True)
            ...
            with gr.TabItem("1C-推理"):
                ...
                with gr.Row():
                    GPT_dropdown = gr.Dropdown(label="*GPT模型列表", choices=sorted(GPT_names,key=custom_sort_key),value=pretrained_gpt_name,interactive=True)
                    SoVITS_dropdown = gr.Dropdown(label="*SoVITS模型列表", choices=sorted(SoVITS_names,key=custom_sort_key),value=pretrained_sovits_name,interactive=True)
